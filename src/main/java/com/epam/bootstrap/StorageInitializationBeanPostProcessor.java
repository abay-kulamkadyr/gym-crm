...
/**
 * * A custom Spring {@link org.springframework.beans.factory.config.BeanPostProcessor} responsible
 * for automatically initializing {@link java.util.Map} beans that are intended to serve as
 * in-memory storage for domain entities. *
 * <p>
 * This processor works by inspecting the {@code @Bean} factory method for the presence of the
 * {@code @InitializableStorage} annotation. If the annotation is found, the component locates the
 * corresponding {@link StorageInitializer} and uses it to populate the newly created Map bean with
 * initial data.
 * </p>
 * *
 * <p>
 * It requires the custom {@code @InitializableStorage} annotation and implementations of the
 * {@code StorageInitializer} interface to function correctly. This approach is typically used for
 * simplified data access layers (DAOs) in a pure in-memory environment, often for testing or simple
...
class StorageInitializationBeanPostProcessor implements BeanPostProcessor, BeanFactoryAware {
  ...
  public StorageInitializationBeanPostProcessor(List<StorageInitializer<?>> initializers) {
    ...
    for (StorageInitializer<?> initializer : initializers) {
      InitializableStorage annotation = initializer.getClass()
          .getAnnotation(InitializableStorage.class);
  ...
  public Object postProcessAfterInitialization(Object bean, String beanName) throws BeansException {
    ...
    if (annotation != null) {
      ...
      if (initializer != null) {
        @SuppressWarnings("unchecked") Map<Long, ?> storage = (Map<Long, ?>) bean;

        log.info("Initializing storage '{}' for type: {}", beanName,
        ...
        log.warn("No initializer found for entity type: {}",
            annotation.entityType().getSimpleName());
      }
    }
      ...
      if (beanDefinition instanceof AnnotatedBeanDefinition annotatedBeanDef) {
        ...
        if (factoryMethodMetadata != null) {
          ...
          if (factoryMethodMetadata.isAnnotated(InitializableStorage.class.getName())) {
            ...
            Map<String, Object> attributes = factoryMethodMetadata.getAnnotationAttributes(
                InitializableStorage.class.getName());
  ...
  /**
   * Creates a proxy implementation of the InitializableStorage annotation since we can't
   * instantiate annotations directly.
   */
  ...
  private <T> void initializeStorageSafely(StorageInitializer<T> initializer,
      Map<Long, ?> storage) {
