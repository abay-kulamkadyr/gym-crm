...
class TrainingControllerTest {
	...
	void testAddTraining_Success() throws Exception {
		...
		AddTrainingRequest request = new AddTrainingRequest("john.doe", "jane.smith", "Morning Workout",
				LocalDateTime.now().plusDays(7), 60);

		when(gymFacade.createTraining(any(CreateTrainingRequest.class))).thenReturn(null);

		// When & Then
		mockMvc
			.perform(post("/api/trainings").contentType(MediaType.APPLICATION_JSON)
	...
	void testAddTraining_LongTrainingName() throws Exception {
		...
		AddTrainingRequest request = new AddTrainingRequest("john.doe", "jane.smith", longName,
				LocalDateTime.of(2024, 12, 1, 9, 0), 60);
	...
	void testAddTraining_NullTrainingDate() throws Exception {
		...
		// When & Then
		mockMvc.perform(post("/api/trainings").contentType(MediaType.APPLICATION_JSON).content(requestJson))
			.andExpect(status().isBadRequest())
			.andExpect(jsonPath("$.error").value("Validation Failed"));
	...
	void testAddTraining_DurationTooShort() throws Exception {
		// Given
		AddTrainingRequest request = new AddTrainingRequest("john.doe", "jane.smith", "Morning Workout",
				LocalDateTime.of(2024, 12, 1, 9, 0), 0 // Invalid: less than 1 minute
		);
	...
	void testAddTraining_MinimumDuration() throws Exception {
		// Given
		AddTrainingRequest request = new AddTrainingRequest("john.doe", "jane.smith", "Quick Session",
				LocalDateTime.now().plusDays(7), 1 // Minimum valid: 1 minute
		);
